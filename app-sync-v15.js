...
const SERVICE_WORKER_URL = "./service-worker.js?v=20260320-15";
...
const dom = {
  ...
  grocerySourceCurrent: document.getElementById("grocery-source-current"),
  grocerySourceAll: document.getElementById("grocery-source-all"),
  grocerySummary: document.getElementById("grocery-summary"),
  groceryList: document.getElementById("grocery-list"),
  copyGroceryList: document.getElementById("copy-grocery-list"),
  clearGroceryChecks: document.getElementById("clear-grocery-checks"),
...
function init() {
  ...
  dom.grocerySourceCurrent.addEventListener("click", () => setGrocerySource("current"));
  dom.grocerySourceAll.addEventListener("click", () => setGrocerySource("all"));
  dom.copyGroceryList.addEventListener("click", copyGroceryListToClipboard);
  dom.clearGroceryChecks.addEventListener("click", clearGroceryChecks);
...
function normalizeState(raw) {
  ...
  return {
    ...
    grocery: {
      source: normalizeGrocerySource(baseState?.grocery?.source),
      checkedKeys: normalizeCheckedKeys(baseState?.grocery?.checkedKeys),
    },
    picker: {
...
function renderGroceryPanel() {
  const grocerySource = normalizeGrocerySource(state.grocery.source);
  const sourceRecipes = getGrocerySourceRecipes();
  const groceryItems = buildGroceryItems(sourceRecipes);
  const checkedCount = groceryItems.filter((item) => item.checked).length;

  dom.grocerySourceCurrent.classList.toggle("is-active", grocerySource === "current");
  dom.grocerySourceAll.classList.toggle("is-active", grocerySource === "all");
  dom.grocerySourceCurrent.setAttribute("aria-pressed", String(grocerySource === "current"));
  dom.grocerySourceAll.setAttribute("aria-pressed", String(grocerySource === "all"));
  dom.grocerySourceCurrent.classList.add("grocery-toggle");
  dom.grocerySourceAll.classList.add("grocery-toggle");

  dom.groceryList.replaceChildren();

  if (!groceryItems.length) {
    const emptyCopy = document.createElement("p");
    emptyCopy.className = "empty-copy";
    emptyCopy.textContent = getEmptyGroceryCopy(grocerySource);
    dom.groceryList.append(emptyCopy);
    dom.grocerySummary.textContent = "No grocery items yet.";
    return;
  }

  dom.grocerySummary.textContent =
    `${groceryItems.length} grocery item${pluralize(groceryItems.length)} from ${sourceRecipes.length} recipe${pluralize(sourceRecipes.length)}. ${checkedCount} checked off.`;
  ...
  for (const item of groceryItems) {
    ...
    checkbox.addEventListener("change", () => toggleGroceryItem(item.key));

    const copy = document.createElement("div");
    copy.className = "grocery-item-copy";
    ...
    meta.textContent = item.recipeTitles.join(", ");

    const count = document.createElement("span");
    count.className = "grocery-item-count";
    count.textContent = item.count > 1 ? `${item.count}x` : "1x";

    copy.append(name, meta);
    row.append(checkbox, copy, count);
    dom.groceryList.append(row);
  }
}

function getCurrentRecipes() {
  return state.cycle.currentPickIds
    .map((recipeId) => state.recipes.find((recipe) => recipe.id === recipeId))
    .filter(Boolean);
}

function getGrocerySourceRecipes() {
  if (normalizeGrocerySource(state.grocery.source) === "all") {
    return state.recipes;
  }

  return getCurrentRecipes();
}

function buildGroceryItems(recipes) {
  const checkedKeys = new Set(normalizeCheckedKeys(state.grocery.checkedKeys));
  ...
  for (const recipe of recipes) {
    for (const ingredient of recipe.ingredients) {
      ...
      item.count += 1;
      if (!item.recipeTitles.includes(recipe.title)) {
        item.recipeTitles.push(recipe.title);
      }
    }
  }

  return [...groceryMap.values()].sort((left, right) => {
    const checkedDiff = Number(left.checked) - Number(right.checked);
    if (checkedDiff) {
      return checkedDiff;
    }
...
function getEmptyGroceryCopy(source) {
  if (!state.recipes.length) {
    return "Add recipes with ingredients and the grocery list will build itself here.";
  }

  if (source === "current") {
    return "Generate today's meals first to build a grocery list from the current picks.";
  }

  return "No saved ingredients were found in the current recipe library.";
}

function setGrocerySource(source) {
  const nextSource = normalizeGrocerySource(source);
  if (state.grocery.source === nextSource) {
    return;
  }

  state.grocery.source = nextSource;
  saveState();
  renderGroceryPanel();
}
...
function toggleGroceryItem(key) {
  ...
  state.grocery.checkedKeys = [...checkedKeys];
  saveState();
  renderGroceryPanel();
}

function clearGroceryChecks() {
  if (!normalizeCheckedKeys(state.grocery.checkedKeys).length) {
    return;
  }

  state.grocery.checkedKeys = [];
  saveState();
  renderGroceryPanel();
}

async function copyGroceryListToClipboard() {
  const groceryItems = buildGroceryItems(getGrocerySourceRecipes());
  if (!groceryItems.length) {
    setShareFeedback("There are no grocery items to copy yet.", "warn");
    return;
  }

  const lines = groceryItems.map((item) =>
    item.count > 1 ? `${item.name} (${item.count} recipes)` : item.name,
  );
  const copied = await copyText(lines.join("\n"));

  if (copied) {
    setShareFeedback("Grocery list copied.", "ok");
    return;
  }

  window.prompt("Copy this grocery list.", lines.join("\n"));
  setShareFeedback("Grocery list ready to copy.", "ok");
}
